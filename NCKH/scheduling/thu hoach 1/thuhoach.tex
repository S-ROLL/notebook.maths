...
Với $k_j = \frac{w_j}{p_j}$ (hay $k_j=\frac{p_j}{w_j}$), nếu bài toán chưa tối ưu thì việc sắp xếp các công việc theo thứ tự giảm dần (hay tăng dần) theo giá trị $k_j$ sẽ giúp bài toán tối thiểu được hàm mục tiêu $\sum w_j C_j$.
\end{dl}
...
\begin{cm}
	...
	\begin{split}
		w_j C_j + w_k C_k &= w_j(t+p_j) + w_k(t+p_j+p_k) > w_k(t+p_k) + w_j(t+p_k+p_j) \\
